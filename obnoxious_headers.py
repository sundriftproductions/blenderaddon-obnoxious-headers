...
bl_info = {
    ...
    "blender" : (2, 93, 0),
    "location" : "UI",
    "description" : "If the automatic keyframe feature is enabled, turn the header color red. If X-Axis Mirror is turned on, Preserve Strands Length (when in Particle Edit mode) is off, or Preserve Root Position (when in Particle Edit mode) is off, obnoxious text message will appear in the lower left hand corner of every 3D view window. This code is a modification of the original header_color_change script by Bookyakuno: https://github.com/bookyakuno/-Blender-/blob/master/header_color_change.py",
    "category" : "UI",
